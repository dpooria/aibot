...
from aibot_date import (
    df_event,
    export_date,
    format_jalali_date,
    gregorian_to_jalali,
    tr_isoweek_toperweekday,
)
from aibot_utils import cleaning, concatenate_bi, unique_without_sort
from reply_gen import tr_gregorian_date, tr_hijri_date, tr_single_date
from timeAPI import Time
from vocab import convert_asked, event_asked, time_asked, week_days, week_days_asked
            ...
            if d_n == 1:
                ...
                if asingle is not None:
                    ...
                    if self.bii:
                        ...
                        if date_list[0].date() >= today.date():
                            generated_sentence = "تاریخ داده شده {} است".format(trsd)
                        else:
                            generated_sentence = "تاریخ داده شده {} بوده".format(trsd)
            else:
                answer["result"] = []
    ...
    def get_single_answer(self, question, answer, date_list, events, bii=None):
        ...
        if not events:
            ...
            if is_event_asked:
                ...
                f_event = df_event["event"].loc[df_event["j_d"] == d_str].to_numpy()
                if not len(f_event) == 0:
                    res = []
                    for e in f_event:
                        res.append(
                            e.replace(" و ", "-").replace("؛", "-").replace("،", "-")
                        )
                    res = np.unique(res)
