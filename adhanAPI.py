...
from vocab import hours_left_asked, adhan_logical_question, tr_adhan_names
from copy import copy
from reply_gen import tr_date, tr_single_date, tr_single_time
...
class Adhan:
    ...
    def get_city_adhanـoneMonth(self, cityName, date):
        ...
        url = self.api_url.format(
            lat=city_coord["coord"]["lat"],
            long=city_coord["coord"]["lon"],
            month=date.month,
            year=date.year,
        )
        try:
    ...
    def get_difference_adhan(self, city1, city2, date1, date2, adhan1, adhan2):
        ...
        if res1 is None or res2 is None:
            return None, url
        res = abs(
            datetime.datetime.combine(date1.date(), res1)
        ...
        minutes = (res // 60) % 60
        if minutes != 0:
            generated_text = "{} ساعت و {} دقیقه".format(int(hours), int(minutes))
        else:
    ...
    def get_answer(self, question, tokens, labels):
        ...
        if n_adhan == 1 and l_n == 1 and d_n == 1:
            ...
            isLogical = False
            for ll in adhan_logical_question:
                if ll in tokens:
                    isLogical = True
            if isLogical:
                if n_adhan > 1 and l_n == 1 and d_n == 1:
                    ...
                    generated_sentence = (
                        "اختلاف زمان {} {} و {} {} {}، {} میباشید".format(
                            exportedAdhan[0],
                            tr_single_date(date_list[0]),
                            exportedAdhan[1],
                            tr_single_date(date_list[1]),
                            location[0],
                            gd,
                        )
                    )
                ...
                if n_adhan >= 2 and l_n == 1 and d_n == 1:
                    ...
                    generated_sentence = "{} ".format(tr_single_date(date_list[0]))
                    ...
                    generated_sentence = "{} {}، ".format(exportedAdhan[0], location[0])
                        ...
                        if res is not None and url is not None:
                            ...
                            if i < d_n - 2:
                                generated_sentence = (
                                    generated_sentence
                                    + "{}، {}، ".format(
                                        tr_single_date(dat), tr_single_time(res)
                                    )
                                )
                            elif i == d_n - 2:
                                generated_sentence = (
                    ...
                    for ad, lc in zip(exportedAdhan, location):
                        res, url = self.get_city_adhan_time(lc, date_list[0].date(), ad)
                        if res is not None and url is not None:
                            res_list.append(res.strftime("%H:%M"))
                            url_list.append(url)
                            if s < l_n - 2:
                                generated_sentence = (
                                    generated_sentence
                                    + "{} {}، {}، ".format(ad, lc, tr_single_time(res))
                                )
                            elif s == l_n - 2:
                                generated_sentence = (
                                    generated_sentence
                                    + "{} {}، {} و ".format(ad, lc, tr_single_time(res))
                                )
                            else:
                                generated_sentence = (
                                    generated_sentence
                    ...
                    for ad, dat in zip(exportedAdhan, date_list):
                        res, url = self.get_city_adhan_time(location[0], dat.date(), ad)
                        if res is not None and url is not None:
                            res_list.append(res.strftime("%H:%M"))
                            url_list.append(url)
                            if s < d_n - 2:
                    ...
                    generated_sentence = generated_sentence + "به افق {} میباشد".format(
                        location[0]
                    )
